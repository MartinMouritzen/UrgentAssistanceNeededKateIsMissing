...
function getLookAndFeelPath() {
	return 'icons/nanotree';
	if (lookAndFeel == WINDOWS_XP) {
		return "winxp";
	}
	else if (lookAndFeel == MAC_OS_X) {
		return "osx";
	}
}
...
function showTree(path,container) {
	readStates();
...
function showNode(treeNode,lastNode) {
	...
	var str;
	str = '<div class="nodeLine" style="filter:alpha(opacity=100);" ondragenter="dragEnter(' + treeNode.getID() + ');" ondragleave="dragLeave();" ondragstart="startDrag(' + treeNode.getID() + ');" ondrag="dragMove();" ondragend="endDrag(' + treeNode.getID() + ')" id="node' + treeNode.getID() + '">';
	str += '<nobr>';
	for(var y=0;y<linestring.length;y++) {
		if (linestring.charAt(y) == 'I') {
			str += '<img src="images/' + getLookAndFeelPath() + '/' + (showLines ? 'line' : 'white') + '.gif" style="width:19px;height:20px;vertical-align:middle;">';
		}
		else if (linestring.charAt(y) == 'B') {
			str += '<img src="images/' + getLookAndFeelPath() + '/white.gif" style="width:19px;height:20px;vertical-align:middle;">';
		}
	}
	if (treeNode.hasChilds()) {
		...
		if (!showRootNode && (treeNode.getParent() == rootNode) && (treeNode.getParent().getFirstChild() == treeNode)) {
			if (!lastNode) {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (state == 'open' ? (showLines ? 'minus_no_root' : 'minus_nolines') : (showLines ? 'plus_no_root' : 'plus_nolines')) + '.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
			else {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (state == 'open' ? 'minus_last' : 'plus_last') + '_no_root.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
		}
		else {
			if (!lastNode) {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (state == 'open' ? (showLines ? 'minus' : 'minus_nolines') : (showLines ? 'plus' : 'plus_nolines')) + '.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
			else {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (state == 'open' ? (showLines ? 'minus_last' : 'minus_nolines') : (showLines ? 'plus_last' : 'plus_nolines')) + '.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
		}
	}
	else {
		...
		if (!showRootNode && (treeNode.getParent() == rootNode) && (treeNode.getParent().getFirstChild() == treeNode)) {
			if (!lastNode) {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (showLines ? 't_no_root' : 'white') + '.gif" style="width:19px;height:20px;vertical-align:middle;">';
			}
			else {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/white.gif" style="width:19px;height:20px;vertical-align:middle;">';
			}
		}
		else {
			if (!lastNode) {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (showLines ? 't' : 'white') + '.gif" style="width:19px;height:20px;vertical-align:middle;">';
			}
			else {
				str += '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (showLines ? 'lastnode' : 'white') + '.gif" style="width:19px;height:20px;vertical-align:middle;">';
			}
		}
	}
...
function selectNode(nodeID) {
	...
	// alert('debug1: ' + nodetitle.className);
...
function refreshNode(treeNode) {
	...
	if (treeNode.getChildCount() == 0) {
		...
		if (actionimage.src.indexOf('last') == -1) {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + (showLines ? 't' : 'white') + '.gif';
		}
		else {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + (showLines ? 'lastnode' : 'white') + '.gif';
		}
		...
		if (actionimage.src.indexOf('plus') != -1) {
			...
			if (actionimage.src.indexOf('last') == -1) {
				actionimage.outerHTML = '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + (showLines ? 'plus' : 'plus_nolines') + '.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
			else {
				actionimage.outerHTML = '<img id="handler' + treeNode.getID() + '" src="images/' + getLookAndFeelPath() + '/' + 'plus_last.gif" style="width:19px;height:20px;vertical-align:middle;" OnClick="handleNode(' + treeNode.getID() + ');">';
			}
		}
	}
...
function handleNode(nodeID) {
	...
	if (submenu.style.display == 'none') {
		...
		if (actionimage.src.indexOf('last') == -1) {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + ((firstChildOfRoot) ? 'minus_no_root' : (showLines ? 'minus' : 'minus_nolines')) + '.gif';
		}
		else {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + ((firstChildOfRoot) ? 'minus_last_no_root' : (showLines ? 'minus_last' : 'minus_nolines')) + '.gif';
		}
	}
	else {
		...
		if (actionimage.src.indexOf('last') == -1) {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + ((firstChildOfRoot) ? 'plus_no_root' : (showLines ? 'plus' : 'plus_nolines')) + '.gif';
		}
		else {
			actionimage.src = href + 'images/' + getLookAndFeelPath() + '/' + ((firstChildOfRoot) ? 'plus_last_no_root' : (showLines ? 'plus_last' : 'plus_nolines')) + '.gif';
		}
	}
}
...
/**
* The high number is to support Opera 7 (I don't know if this is a bug, it's certainly a weird number)
*/
function keyDown(event) {
	if (window.event) {
		event = window.event;
	}
	if (event.keyCode == 38 || event.keyCode == 57373) { // Up
		selectPrevNode();
		return false;
	}
	else if (event.keyCode == 40 || event.keyCode == 57374) { // Down
		selectNextNode();
		return false;
	}
	else if (event.keyCode == 37 || event.keyCode == 57375) { // left
		subtractNode();
		return false;
	}
	else if (event.keyCode == 39 || event.keyCode == 57376) { // right
		expandNode();
		return false;
	}
}
